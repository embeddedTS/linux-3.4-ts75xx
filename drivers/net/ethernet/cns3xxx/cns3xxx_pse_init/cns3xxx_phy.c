...
/* Why oh why oh why did someone write 'get_phy_id' when it conflicts
   with a function of the same name elsewhere? 
   */
   
static u16 __get_phy_id(u8 phy_addr);
...
#endif
{
	...
	phy_data = __get_phy_id(phy_addr); // should be 0x243
...
int icp_101a_init(u8 mac_port, u8 phy_addr)
{
	...
	phy_data = __get_phy_id(mac_port);
	if (phy_data != 0x0243) {
