...
int cns3xxx_config_VSC8601(u8 mac_port, u8 phy_addr)
{
	...
	if (phy_id != 7) {	printk("VSC8601 phy id %x is not correct.  Maybe its a LAN8710i...\n", phy_id);
		//return CAVM_ERR;
   }
   
   if (phy_id != 0x8000) {
      printk("Nope, not a LAN8710i, giving up\n");
      
         return CAVM_ERR;
	}
   else
      printk("Yep, it's a LAN8710i\n");
