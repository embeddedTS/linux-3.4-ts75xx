...
static int cns3xxx_sidpr_set_lpm(struct ata_link *link, enum ata_lpm_policy policy,
			      unsigned hints);
...
/* SATA offload control flag */
extern struct proc_dir_entry * cns3xxx_proc_dir;
extern int cnsx_sata_offload;
struct proc_dir_entry *sata_offload_proc_entry;
#endif
...
static ssize_t ahci_transmit_led_message(struct ata_port *ap, u32 state,
					ssize_t size);
#if defined(CONFIG_AMP_PURENAS) || defined(CONFIG_AMP_ALL)
static void sata_offload_proc_init(void);
#endif
...
enum {
   ...
   */
   AHCI_FLAG_COMMON		= ATA_FLAG_SATA  |
				      ATA_FLAG_PIO_DMA |
				      ATA_FLAG_ACPI_SATA | ATA_FLAG_AN, ATA_FLAG_IPM, 
...
#ifndef CONFIG_MACH_TS43XX
   /* None of this is any use on the ts43xx */
	[board_ahci_vt8251] =
	{
...
static void ahci_enable_ahci(void __iomem *mmio)
{
	...
	/* turn on AHCI_EN (if not already on)*/
...
static int ahci_scr_read(struct ata_link *link, unsigned int sc_reg, u32 *val)
{
	...
	if (offset) {
		*val = readl(port_mmio + offset);
		...
		printk("ahci_scr_read 0x%08lX = 0x%08lX\n", (unsigned long)(port_mmio + offset), (unsigned long)*val);
...
static int ahci_scr_write(struct ata_link *link, unsigned int sc_reg, u32 val)
{
	...
	printk("ahci_scr_write 0x%08lX 0x%08lX\n", (unsigned long)(port_mmio + offset), (unsigned long)val);
...
static int ahci_reset_controller(struct ata_host *host)
{
	...
	if (!ahci_skip_host_reset) {
		 ...
		 */
		 ...
		 printk("polling at 0x%08lX\n", (unsigned long)(mmio + HOST_CTL));
...
static void ahci_init_controller(struct ata_host *host)
{
	...
	/* In fact, cns3xxx_cpu_clock() returns the CPU clock / 1000000, so
	   for as 600MHz CPU clock, it returns 600.  And, since hclk is half of
	   the cpu clock, we need to put (600*500) into the register below,
	   because 300000000 / 300000 = 1000uS  */
			  ...
			  unsigned long deadline)
{
	...
	if ( pmp && ret ) return ahci_do_softreset(link, class, 0, deadline, ahci_check_ready);
	return -1;
}
#if (0)
...
#if defined(CONFIG_AMP_PURENAS) || defined(CONFIG_AMP_ALL)
	remove_proc_entry(sata_offload_proc_entry->name, cns3xxx_proc_dir);
#endif
...
#endif
{
	...
	for (i = 0; i < host->n_ports; i++) {
		...
		//ap->pm_policy = NOT_AVAILABLE; XXX FIX ME
...
static void ahci_phy_init(void){
	...
	u32tmp &= ~(0x1<< 16);  // Put SATA PHY 0 into SLUMBER Mode	
#else
...
#if defined(CONFIG_AMP_PURENAS) || defined(CONFIG_AMP_ALL)
static int sata_offload_read_proc(char *page, char **start, off_t off, 
				  int count, int *eof, void *data)
{
	int num = 0;

	num += sprintf(page + num, "sata_offload = %d\n", cnsx_sata_offload);
    
	return num;
}

static int sata_offload_write_proc(struct file *file, const char *buffer, 
				unsigned long count, void *data)
{
	long val = simple_strtol(buffer, NULL, 10);
	cnsx_sata_offload = val ? 1 : 0;
	return count;
}

static void sata_offload_proc_init()
{
	sata_offload_proc_entry = create_proc_entry("sata_offload", 
						    S_IFREG | S_IRUGO, 
						    cns3xxx_proc_dir);
	if (sata_offload_proc_entry) {
		sata_offload_proc_entry->read_proc = sata_offload_read_proc;
		sata_offload_proc_entry->write_proc = sata_offload_write_proc;
	}
}
#endif
