...
static void __init cns3xxx_pcie_hw_init(struct cns3xxx_pcie *cnspci)
{
   ...
   /* enable memory access to the FPGA's registers (if we don't do it here,
      then we need to do it in userspace, and that's slower) */
	pci_bus_read_config_word(&bus, devfn, PCI_COMMAND, &dc);
	dc |= 0x407;
	pci_bus_write_config_word(&bus, devfn, PCI_COMMAND, dc);
#endif
...
#ifdef CONFIG_MACH_TS43XX	
   printk("TS-43XX Warning: Disabling Completion Timeout in PCIe\n");

   *(volatile unsigned long *)(cns3xxx_pcie[0].cfg_bases[CNS3XXX_HOST_TYPE].virtual + 0x98) = 0x10;
  	
   printk("Write to 0x%08lX\n", 
      (volatile unsigned long *)(cns3xxx_pcie[0].cfg_bases[CNS3XXX_HOST_TYPE].virtual + 0x98));
      
	//*(volatile unsigned long *)(CNS3XXX_PCIE0_HOST_BASE_VIRT + 0x98) = 0x10;
#endif	
